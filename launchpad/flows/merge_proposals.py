...
async def extract_data(query: LaunchpadQuery) -> List[Dict[str, Any]]:
    ...
    for merge_proposal in merge_proposals:
        ...
        events.extend(extract_merge_proposal_events(query, merge_proposal, time_zone, from_date, to_date))
...
async def extract_data_streaming_enhanced(query: LaunchpadQuery, chunk_size: int) -> AsyncIterator[List[Dict[str, Any]]]:
    ...
    for i in range(0, len(merge_proposals), chunk_size):
        ...
        for merge_proposal in batch:
            events_batch.extend(extract_merge_proposal_events(query, merge_proposal, time_zone, from_date, to_date))
...
"""
def extract_merge_proposal_events(query: LaunchpadQuery, merge_proposal, time_zone: str, from_date: datetime, to_date: datetime) -> List[Dict[str, Any]]:
    events_batch =[]
    ...
    if not dates_in_range(dates, from_date, to_date): return events_batch # skip if no dates are in range
    ...
    if comments_response.status_code != 200: return events_batch # No comments were found, skip to next merge proposal

    for comment in comments_response.json()['entries']:
        date_created = datetime.strptime(comment['date_created'], "%Y-%m-%dT%H:%M:%S.%f%z")
        if not date_in_range(date_created, from_date, to_date): continue # Skip comments outside the date range
...
def extract_merge_proposal_event_props(merge_proposal) -> dict:
    return {
        'address': merge_proposal.address if hasattr(merge_proposal, 'address') else None,
        'description': merge_proposal.description if hasattr(merge_proposal, 'description') else None,
        'prerequisite_branch_link': merge_proposal.prerequisite_branch_link if hasattr(merge_proposal, 'prerequisite_branch_link') else None,
        'prerequisite_git_repository_link': merge_proposal.prerequisite_git_repository_link if hasattr(merge_proposal, 'prerequisite_git_repository_link') else None,
        'preview_diffs_collection_link': merge_proposal.preview_diffs_collection_link if hasattr(merge_proposal, 'preview_diffs_collection_link') else None,
        'private': merge_proposal.private if hasattr(merge_proposal, 'private') else None,
        'status': merge_proposal.queue_status if hasattr(merge_proposal, 'queue_status') else None,
        'link': merge_proposal.web_link if hasattr(merge_proposal, 'web_link') else None,
        'source_branch_link': merge_proposal.source_branch_link if hasattr(merge_proposal, 'source_branch_link') else None,
        'source_git_repository_link': merge_proposal.source_git_repository_link if hasattr(merge_proposal, 'source_git_repository_link') else None,
        'superseded_by_link': merge_proposal.superseded_by_link if hasattr(merge_proposal, 'superseded_by_link') else None,
        'supersedes_link': merge_proposal.supersedes_link if hasattr(merge_proposal, 'supersedes_link') else None,
        'target_branch_link': merge_proposal.target_branch_link if hasattr(merge_proposal, 'target_branch_link') else None,
        'target_git_repository_link': merge_proposal.target_git_repository_link if hasattr(merge_proposal, 'target_git_repository_link') else None,
    }


def extract_merge_proposal_metrics(merge_proposal) -> dict:
    return {
        'queue_position': merge_proposal.queue_position if hasattr(merge_proposal, 'queue_position') and merge_proposal.queue_position else None,
    }


def extract_merge_proposal_reviewed_relation_props(merge_proposal) -> dict:
    return {
        'reviewed_revid': merge_proposal.reviewed_revid if hasattr(merge_proposal, 'reviewed_revid') else None,
    }


def extract_merge_proposal_merged_relation_props(merge_proposal) -> dict:
    return {
        'merged_revision_id': merge_proposal.merged_revision_id if hasattr(merge_proposal, 'merged_revision_id') else None,
        'merged_revno': merge_proposal.merged_revno if hasattr(merge_proposal, 'merged_revno') else None,
    }
