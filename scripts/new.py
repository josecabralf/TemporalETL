...
def check_and_ensure_query_file(source_dir: Path, source_name: str) -> None:
    ...
    needs_update = (
        missing_imports
        or not has_query_type
        or not has_class
        or not has_from_dict
        or not has_to_summary_base
    )
...
def check_and_ensure_flow_file(
    flows_dir: Path, source_name: str, flow_name: str
) -> None:
    ...
    extract_method_pattern = (
        rf'@extract_method\(name="{re.escape(source_name)}-{re.escape(flow_name)}"\)'
    )
